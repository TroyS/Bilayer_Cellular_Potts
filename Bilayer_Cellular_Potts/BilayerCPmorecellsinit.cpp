...
int i=0, j=0,nall,ncells,confchk, latchk, nx, ny, ns;
...
int ity = 0, ncc, nsub, nctotal = 0;

vector<int> ncelv;

inindx >> words; inindx >> nsub; nctotal = nctotal+nsub; 

ncelv.push_back(nsub);
...
for (ity=0; ity<ncells; ity++)

{ //inindx >> ncc; 
	ncelv.push_back(ncc); nctotal = nctotal+ncc; }
...
for (i=1; i<nall; i++) {celsdat1[i].Ainit = ncelv[i]; celsdat2[i].Ainit = ncelv[i];}
...
if (nsub == 0) {
setgridVoro2(pnts1, gd1, 1, 1);
setgridVoro2(pnts2, gd2, 1, 2); }

if (nsub != 0) {
setgridVoro2(pnts1, gd1, 0, 1);
setgridVoro2(pnts2, gd2, 0, 2); }


//setgrid4(pnts1, gd1, ncelv, 1);
//setgrid4(pnts2, gd2, ncelv, 2);
...
string st1 = to_string(ns); string st2 = to_string(nx);
...
double Perii1[nall] = {0}, Perii2[nall] = {0}, Pp;

double areaf1[nall] = {0}, areaf2[nall] = {0}, Aa;
...
celsdat2[0].CPsetall(0, 0, 0, 0, eps[1], 0, laac[1], mac[1]);
celsdat2[0].lay = 2;
...
celsdat1[i].lay = 1; 
celsdat1[i].CPsetall(lamsa[0], areaf1[i], lamsp[0], Perii1[i], eps[0], epslay[0], laac[0], mac[0]);


celsdat2[i].lay = 2; 
celsdat2[i].CPsetall(lamsa[1], areaf2[i], lamsp[1], Perii2[i], eps[1], epslay[1], laac[1], mac[1]); 
...
string et3 = to_string(areaf1[1]); string et4 = to_string(areaf2[1]);
string et5 = to_string(lamsp[0]); string et6 = to_string(Perii1[1]);
string et7 = to_string(Perii2[1]); string et8 = to_string(epslay[1]);
