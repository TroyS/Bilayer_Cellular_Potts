...
int i = 0,j = 0,x = 0,y = 0,zlay,ntypes,nall,ncells,nx,ns,la,hw,latchk,confchk;
...
double T, Peris1[nall] = {0}, Peris2[nall] = {0}, areas1[nall], areas2[nall],Aa;
...
hexorder(pstart1,celstart1,gd1,1);
hexorder(pstart2,celstart2,gd2,1);
...
stringstream stream;
stream << fixed << setprecision(1) << abs(Bipara);
Bilabel = stream.str();
para = "Bi_" + Bilabel;
