...
//This one will include a distance dependent coupling strength to introduce bending, with the bilayer coupling implemented at the site level
//Here we check region II for the match condition
...
if (argc < 4) 
{
	cout << "Enter the input file name, " << endl;	cin >> infile; 
	cout << "Enter the parameter file name, " << endl;	cin >> pfile; 
	cout << "Enter the output file name, " << endl;	cin >> oindx;
}
...
double Da,Ya,Latt = 1,rad3 = sqrt(3), Pointbi;
...
double bend = Latt/2, centery = gd1.COMy, centerx = gd1.COMx;

//Cellprop celstart1[nall], celstart2[nall];
Cellprop* celstart1 = nullptr; Cellprop* celstart2 = nullptr;
celstart1 = new Cellprop[nall];	celstart2 = new Cellprop[nall];

//Point pstart1[ns],pstart2[ns];

Point* pstart1 = nullptr; Point* pstart2 = nullptr;
pstart1 = new Point[ns];	pstart2 = new Point[ns];
...
double T, Peris1[nall]={0},Peris2[nall]={0}, Pp,areas1[nall], areas2[nall],Aa;
...
//getcom(pstart1,gd1,celstart1,1);
//getcom(pstart2,gd2,celstart2,1);
...
//double switchlen = 1.5*Perii1[1]/M_PI;

//double switchlen1 = 1.5*Peris1[1]/M_PI, switchlen2 = 1.5*Peris2[1]/M_PI;

double switchlen1 = Peris1[1]/M_PI, switchlen2 = Peris2[1]/M_PI, lowbound = centery - 2*switchlen1, midbound = centery-switchlen1, hibound = centery + switchlen1, outbound = centery+2*switchlen1;

//double switchlen1 = 4*sqrt(areas1[1]/M_PI), switchlen2 = 4*sqrt(areas2[1]/M_PI);


i = 0;

for (i=0; i<nall; i++ ) { 
 cout << "The hex area of cell " << i << " in layer 1 is " << celstart1[i].Aihex << " , the hex perimeter is " << celstart1[i].Pihex <<  ", the hexatic order is " << celstart1[i].hexaorder << ", the edge match ratio is " << celstart1[i].matchratio << ", and the ycom is " << celstart1[i].ycom << endl;
	 
cout << endl;

cout << "The hex area of cell " << i << " in layer 2 is " << celstart2[i].Aihex << ", the hex perimeter is " << celstart2[i].Pihex << " , the hexatic order is " << celstart2[i].hexaorder << ", the edge match ratio is  " << celstart2[i].matchratio << ", and the ycom is at " << celstart2[i].ycom << endl;
...
//pointbi = pstart2[j].Bi;
dt1 = to_string(pstart2[j].cnum); dt2 = to_string(xcoor);
dt3 = to_string(pstart2[j].yhex); dt4 = to_string(pstart2[j].Peri);
//dt5 = to_string(pstart2[j].nedge);
//if (pstart2[j].yhex <= switchlen2 )
//	{ dt5 = to_string(2); }
//else if ((pstart2[j].yhex <= 2*switchlen2) && (pstart2[j].yhex > switchlen2 ) )
//	{ dt5 = to_string(1); }
//else
//	{ dt5 = to_string(0); }

dt5 = to_string(0);
dt6 = to_string(pstart2[j].Activ);

datinit2 += dt7 + " " + dt1 + " " + dt2 + " " + dt3 + " 0 0 0 0 " + dt4 + " " + "0" + " " + dt6 + "\n";
...
//Bendlist Blist1, Blist2; 

int swp = 0, nswitch = 1, bichk1 = 0, bichk2 = 0;

//double dswp = (Biend - epslay[0])/Bifreq;

int nswp = abs(ceil((Biend - epslay[0])/Bifreq));

double Bipara,Perimavg,Edgematavg, Matchperiratio,Horder1, Horder2,kend;
...
int biswitch = 0;
double match = 0.3, checkmtch1=0, checkmtch2 = 0, checkmtch3 = 0, checkmtch4 = 0, checkrangelow, checkrangehi;

for (swp = 0; swp < nswp; swp++)

{

//Blist1.initialize(); Blist2.initialize();
...
lowbound = centery-2*switchlen1;
midbound = centery-switchlen1;
hibound = centery+switchlen1;
outbound = centery+2*switchlen1;
...
Point* pnts1 = nullptr; Point* pnts2 = nullptr;
pnts1 = new Point[ns];  pnts2 = new Point[ns];

Cellprop* celsdat1 = nullptr;   Cellprop* celsdat2 = nullptr;
celsdat1 = new Cellprop[nall];  celsdat2 = new Cellprop[nall];
...
{	
celsdat1[i].lay = 1; 
celsdat1[i].CPsetall(lamsa[0], areaf1[i-1], lamsp[0], Perii1[i-1], eps[0], 0, laac[0], mac[0]); 
...
celsdat2[i].CPsetall(lamsa[1], areaf2[i-1], lamsp[1], Perii2[i-1], eps[1], 0, laac[1], mac[1]);

//celsdat2[i].CPsetall(lamsa[1], areaf2[i-1], lamsp[1], Perii2[i-1], eps[1], Bipara, laac[1], mac[1]); 
 
}

//Then we start the bending version of bilayer coupling

//setbendingy(celsdat1, Blist1, nall, switchlen1, 2*switchlen1, Bipara, kend);
//setbendingy(celsdat2, Blist2, nall, switchlen2, 2*switchlen2, Bipara, kend);

nswitch = 0;

setbendingpointy(pnts1, ns, lowbound, midbound, hibound, outbound,Bipara,kend);
setbendingpointy(pnts2, ns, lowbound, midbound, hibound, outbound,Bipara,kend);

//We check if Bipara was set correctly
//cout << "Bipara is " << Bipara << ", and kend is " << kend << endl;
...
et3 = to_string(areaf1[1]); et4 = to_string(areaf2[1]);
...
Perimavg = 0; Edgematavg = 0; Matchperiratio = 0;
	...
	{ 				// The pixel loop
		...
		if (biswitch == 0)
		{
			...
			checkmtch1 = regionbimatchpointy(pnts1,nei1, hibound,outbound);

			checkmtch2 = regionbimatchpointy(pnts2,nei2,hibound,outbound);

			checkmtch3 = regionbimatchpointy(pnts1,nei1, lowbound,midbound);

                        checkmtch2 = regionbimatchpointy(pnts2,nei2,lowbound,midbound);




		if ((checkmtch1>=match) && (checkmtch2 >=match) && (checkmtch3 >=match) && (checkmtch4 >= match)) 
		{
			cout << "Moving the bending region at MCS " << ts << " and ts " << tn << ", region I ends at " << (nswitch+1)*switchlen1 << endl;
			nswitch+=1;
			lowbound = centery - (nswitch+2)*switchlen1;
			midbound = centery - (nswitch+1)*switchlen1;
			hibound = centery + (nswitch+1)*switchlen1;
			outbound = centery + (nswitch+2)*switchlen1;
			setbendingpointy(pnts1, ns, lowbound, midbound, hibound, outbound,Bipara,kend);
			setbendingpointy(pnts2, ns, lowbound, midbound, hibound, outbound,Bipara,kend);


		}	//Finish moving the regions

		if ( (hibound > gd1.Yhi) && ( midbound < gd1.Ylo) ) 
			{
				biswitch = 1; 
			}
...
CPedgealgpointbend(pnts1, pnts2, gd1, celsdat1,celsdat2, Hlay1, T,Latt, nei1);


CPedgealgpointbend(pnts2, pnts1, gd2, celsdat2,celsdat1, Hlay2, T,Latt, nei2);
...
////////////////////////////
		...
		dt1 = to_string(ggs+1+ns); dt2 = to_string(pnts2[ggs].cnum);
                ...
                //dt7 = to_string(pnts2[ggs].nedge);
		Pointbi = pnts2[ggs].Bi;

		if (Pointbi == Bipara)
			{ dt7 = to_string(2); }
		else if ( Pointbi == Bipara-kend )
			{ dt7 = to_string(1); }


		else
			{ dt7 = to_string(0); }
//		dt7 = to_string(std::abs(pointbi));
		dt8=to_string(pnts2[ggs].Activ);
...
//getcom(pnts1,gd1,celsdat1,1);
//getcom(pnts2,gd2,celsdat2,1);
...
for (i=0; i<nall; i++ ) 
{
	cout << "The hex area of cell " << i << " in layer 1 is " << celsdat1[i].Aihex << " , the hex perimeter is " << celsdat1[i].Pihex << " the hexatic order is " << celsdat1[i].hexaorder << ", and the edge match ratio is " << celsdat1[i].matchratio << endl;
...
cout << "The hex area of cell " << i << " in layer 2 is " << celsdat2[i].Aihex << ", the hex perimeter is " << celsdat2[i].Pihex << " , the hexatic order is " << celsdat2[i].hexaorder << ", and the edge match ratio is " << celsdat2[i].matchratio << endl;
...
delete[] celsdat1;	delete[] celsdat2;
celsdat1 = nullptr;	celsdat2 = nullptr;

delete[] pnts1;		delete[] pnts2;
pnts1 = nullptr;	pnts2 = nullptr;
