...
//This will feature two bending regions moving independently
...
double Da,Ya,Latt = 1,rad3 = sqrt(3), Pointbi, sidelen;
...
double Biend; inindx >> words >> Biend; 
double match; inindx >> match;
...
//double switchlen1 = Peris1[1]/M_PI, switchlen2 = Peris2[1]/M_PI;
double switchlen1 = 2*sqrt(2*areas1[1]*rad3)/3, switchlen2 = 2*sqrt(2*areas2[1]*rad3)/3;
double ybotlo = centery - switchlen1, ybothi = centery, ytoplo = centery, ytophi = centery+switchlen1;
...
int biswitchtop = 0, biswitchbot = 0;
double checkmtchtop1=0, checkmtchlow1 = 0, checkmtchtop2 = 0, checkmtchlow2 = 0, checkrangelow, checkrangehi;
...
ybotlo = centery-switchlen1;
ybothi = centery;
ytoplo = centery;
ytophi = centery+switchlen1;
...
setbendingpointysharp(pnts1, ns, centery, ytophi,Bipara);
setbendingpointysharp(pnts1, ns, ybotlo, centery,Bipara);

setbendingpointysharp(pnts2, ns, centery, ytophi,Bipara);
setbendingpointysharp(pnts2, ns, ybotlo, centery,Bipara);
...
et1 = to_string(eps[0]); et2 = to_string(lamsa[0]); 
...
//We also make a file to keep track of the region extensions

extfile = oindx + para + "regextensions.txt";
string exthead = "Numext1 Numext2\n";
string ext1body = "Ext1 ";
string ext2body = "Ext2 ";

Perimavg = 0;
Edgematavg = 0;
Matchperiratio = 0;
int numext1 = 0,numext2 = 0;
...
ts = 0; tn = 0; biswitchtop = 0; biswitchbot = 0;
...
Perimavg = 0; Edgematavg = 0; Matchperiratio = 0;
	...
	{ 				// The pixel loop
		...
		if (biswitchtop == 0)
		{
			...
			checkmtchtop1 = regionbimatchpointy(pnts1,nei1,ytoplo,ytophi);
			checkmtchtop2 = regionbimatchpointy(pnts2,nei2,ytoplo,ytophi);
			


			if ((checkmtchtop1>=match) && (checkmtchtop2 >=match))
                	{
                        	cout << "Moving the top bending region at MCS " << ts << " and ts " << tn << ", region I top ends at " << ytophi+switchlen1 << endl;
                        	ytophi += switchlen1;
                        	ytoplo += switchlen1;
//                      	nswitch+=1;

                        	setbendingpointysharp(pnts1, ns, ytoplo, ytophi,Bipara);
                        	setbendingpointysharp(pnts2, ns, ytoplo, ytophi,Bipara);

                        	numext1 +=1;
                        	ext1body += to_string(ts) + " ";

                	}       //Finish moving the top regions



			if ( (ytophi >= gd1.Yhi ) || (ytoplo >= gd1.Yhi) )
                        	{
                                	biswitchtop = 1;
                        	}
		...
		if (biswitchbot == 0)
			...
			checkmtchlow1 = regionbimatchpointy(pnts1,nei1,ybotlo,ybothi);


			checkmtchlow2 = regionbimatchpointy(pnts2,nei2,ybotlo,ybothi);




			if ((checkmtchlow1>=match) && (checkmtchlow2 >=match)) 
			{
				cout << "Moving the bottom bending region at MCS " << ts << " and ts " << tn << ", region I ends at " << ybotlo-switchlen1 << endl;
				ybothi -= switchlen1;
				ybotlo -= switchlen1;

				setbendingpointysharp(pnts1, ns, ybotlo, ybothi,Bipara);
				setbendingpointysharp(pnts2, ns, ybotlo, ybothi,Bipara);


				numext2 +=1;
                        	ext2body += to_string(ts) + " ";


			}	//Finish moving the bottom regions



			if ( ( ybotlo <= gd1.Ylo)  || (ybothi <= gd1.Ylo) )  
			{
				biswitchbot = 1; 
			}
...
ofstream benfile(extfile.c_str());
